					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$paycategory = $conn->query("SELECT * FROM payment_cat order by id asc");
								while($row=$paycategory->fetch_assoc()):
								?>
								<tr>
									<td class="text-center"><?php echo $i++ ?></td>
									<td class="">
...
<script>
	...
	$('#manage-payment.category').submit(function(e){
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_paycategory',
			data: new FormData($(this)[0]),
		    cache: false,
		    contentType: false,
		    processData: false,
	...
	function delete_paycategory($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_paycategory',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
